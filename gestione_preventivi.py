...
def mostra_interfaccia_preventivi(scarica_da_sheet, carica_su_sheet, invia_email_sistema, url_intermediario, df_istanze, genera_pdf_ordine_fornitore=None, carica_su_drive_unico=None, id_cartella_ordini=None):
    ...
    # --- TAB 4: DETTAGLIO ARTICOLI DINAMICO E LETTERA D'ORDINE ---
    with tab_registro_finito:
        ...
        if df_preventivi_validi.empty:
            ...
            if preventivi_valutabili.empty:
                ...
                # --- SISTEMA DINAMICO AGGIUNTA RIGHE ARTICOLI ---
                st.markdown("##### 🛒 Composizione Righe Articoli (Dettaglio Preventivo)")
                ...
                if st.button("🔴 GENERA LETTERA D'ORDINE CON ARTICOLI DETTAGLIATI", type="primary", use_container_width=True):
                    ...
                    for art in lista_articoli:
                        if not art["descrizione"].strip():
                            errori = True
                    if errori:
                        st.error("Errore: Assicurati di aver inserito la descrizione per tutte le righe articolo attivate.")
                        ...
                        mappa_dati_pdf = {
                            ...
                            "articoli": lista_articoli, # Passiamo l'intera lista strutturata
                        ...
                        # Salviamo il dettaglio articoli compattato come testo note per non alterare le colonne del foglio Excel/Sheet
