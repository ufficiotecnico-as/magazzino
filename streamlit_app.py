...
# --- LETTERA D'ORDINE CON TABELLA ARTICOLI MULTIPLI ---
def genera_pdf_ordine_fornitore(dati_ordine):
    ...
    testo_rif = (f"In riferimento a Vostro preventivo inviato in data {dati_ordine.get('data_preventivo')} "
                 ...
                 f"si invia ordine per quanto segue:")
    pdf.multi_cell(180, 6, pulisci_caratteri_fpdf(testo_rif))
    pdf.ln(8)
    
    # --- INTESTAZIONE TABELLA ---
    pdf.set_font("Times", "B", 10)
    pdf.cell(100, 7, pulisci_caratteri_fpdf("Descrizione Prodotto/Bene"), border=1, ln=False, align="L")
    pdf.cell(15, 7, pulisci_caratteri_fpdf("Q.ta'"), border=1, ln=False, align="C")
    pdf.cell(30, 7, pulisci_caratteri_fpdf("Prezzo Un. IVATO"), border=1, ln=False, align="R")
    pdf.cell(35, 7, pulisci_caratteri_fpdf("Totale IVATO"), border=1, ln=True, align="R")
    
    # --- CICLO RIGHE ARTICOLI DINAMICI ---
    pdf.set_font("Times", "", 10)
    articoli = dati_ordine.get("articoli", [])
    for art in articoli:
        pdf.cell(100, 7, pulisci_caratteri_fpdf(art.get("descrizione", "")), border=1, ln=False, align="L")
        pdf.cell(15, 7, pulisci_caratteri_fpdf(str(art.get("quantita", 1))), border=1, ln=False, align="C")
        pdf.cell(30, 7, pulisci_caratteri_fpdf(f"{art.get('prezzo_unitario')} EUR"), border=1, ln=False, align="R")
        pdf.cell(35, 7, pulisci_caratteri_fpdf(f"{art.get('totale_riga')} EUR"), border=1, ln=True, align="R")
    
    # --- TOTALE COMPLESSIVO ---
    pdf.set_font("Times", "B", 10)
    pdf.cell(145, 7, pulisci_caratteri_fpdf("TOTALE COMPLESSIVO DELL'ORDINE (IVATO)"), border=1, ln=False, align="R")
    pdf.cell(35, 7, pulisci_caratteri_fpdf(f"{dati_ordine.get('importo_ivato')} EUR"), border=1, ln=True, align="R")
    pdf.ln(10)
    ...
    testo_legge = ("Per ottemperare agli obblighi previsti dalla legge Vi ricordiamo che sarà necessario rilasciare la "
                   "\"DICHIARAZIONE RELATIVA AL POSSESSO DEI REQUISITI...\" completa di carta d'identità "
            ...
            with st.form("form_fabb_standard"):
                ...
                if st.form_submit_button("Invia Segnalazione Fabbisogno"):
                    if mat.strip():
                        ...
                        st.error("Specificare il materiale.")import streamlit as st
import pandas as pd
from datetime import datetime
import io
import base64
