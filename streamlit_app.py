...
# Librerie di archiviazione Google (Protette da try-except)
try:
    import gspread
...
def genera_pdf_comodato(id_contratto, nome, ruolo, bene, data, tipo_operazione, firma_base64=None, utente_loggato="Ufficio Tecnico"):
    ...
    # Decodifica e inserimento della firma Base64 nel PDF
        ...
        with tab_comodati:
            ...
            with sub_nuovo:
                ...
                if df_inv_comodati.empty or not (df_inv_comodati["stato"] == "Disponibile").any():
                    ...
                    # --- CANVAS REALE HTML5 TRASFERITO CON PASSAGGIO DATI SICURO ---
                    firma_html = """
                    <div style="background: white; border: 2px dashed #cbd5e1; padding: 10px; border-radius: 12px; width:530px;">
                        <canvas id="canvas_firma" width="500" height="150" style="border:1px solid #94a3b8; background:#fff; cursor:crosshair; touch-action: none;"></canvas>
                        <br>
                        <button onclick="clearCanvas()" style="margin-top:8px; padding:6px 12px; background:#64748b; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:600;">Cancella e Rifai</button>
                    </div>
                    <script>
                        ...
                        function startDraw(e) { drawing = true; var p = getPos(e); ctx.beginPath(); ctx.moveTo(p.x, p.y); e.preventDefault(); }
                        function draw(e) { if(!drawing) return; var p = getPos(e); ctx.lineTo(p.x, p.y); ctx.stroke(); sendData(); e.preventDefault(); }
                        ...
                        function clearCanvas() { ctx.clearRect(0, 0, canvas.width, canvas.height); window.parent.postMessage({type: 'firma', data: ''}, '*'); }
                        function sendData() { window.parent.postMessage({type: 'firma', data: canvas.toDataURL('image/png')}, '*'); }
                    </script>
                    """
                    st.components.v1.html(firma_html, height=220)
                    
                    # Campo di acquisizione invisibile ma sincronizzato
                    stringa_firma = st.text_input("Token Firma Attivo (Non modificare)", type="password", key="token_firma_nascosto")
                    ...
                    if st.button("✍️ Approva e Salva PDF su Google Drive", type="primary", use_container_width=True):
                        ...
                        if nome_pulito:
                            # Controlliamo se la stringa della firma contiene dati effettivi
                            if len(stringa_firma) < 100:
                                ...
                                with st.spinner("Generazione ed upload del documento in corso..."):
                                    ...
                                    # Genera file PDF passando la stringa Base64
                                    pdf_output_bytes = genera_pdf_comodato(id_com, nome_pulito, tipo_sog, bene_sel, data_ora, "CONSEGNA", stringa_firma, st.session_state.utente_corrente)
                                    ...
                                    if carica_su_drive_unico(pdf_output_bytes, nome_file_pdf, "application/pdf", "Comodati_Consegne"):
                                        ...
                                        st.error("Impossibile caricare su Drive. Verifica le credenziali cloud nei Secrets.")
                        else:
