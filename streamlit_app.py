...
if st.session_state.ruolo_utente is None:
    ...
    # --- AREA MAGAZZINIERE ---
        ...
        with tab_carico:
            ...
            # MOTORE DI SCANSIONE POTENZIATO CON FORMATO RETTANGOLARE E AUTOFOCUS
            ...
            <div style="background: #ffffff; padding: 12px; border-radius: 12px; border: 2px dashed #475569; text-align: center;">
                ...
                <p id="scanner-output" style="font-family: system-ui, sans-serif; color: #1e293b; font-weight: 800; margin-top: 12px; font-size: 1.1rem; background: #e2e8f0; padding: 8px; border-radius: 6px;">📷 Inquadra tenendo il codice orizzontale nel rettangolo</p>
            </div>
            
            <script src="https://unpkg.com/html5-qrcode"></script>
            <script>
                // Funzione per calcolare l'area di scansione ottimale per codici a barre (Larga e Bassa)
                let qrboxFunction = function(viewfinderWidth, viewfinderHeight) {
                    let minEdgePercentage = 0.75; 
                    let boxWidth = Math.floor(viewfinderWidth * minEdgePercentage);
                    let boxHeight = Math.floor(boxWidth / 2.5); // Rapporto rettangolare perfetto per barcode lineari
                    return {
                        width: boxWidth,
                        height: boxHeight
                    };
                ...
                const config = { 
                    fps: 30, // Aumentato il campionamento hardware
                    qrbox: qrboxFunction,
                    experimentalFeatures: {
                        useBarCodeDetectorIfSupported: true // Sblocca la decodifica nativa hardware di Android/iOS se disponibile
                    },
                    formatsToSupport: [ 
                ...
                function onScanSuccess(decodedText, decodedResult) {
                    document.getElementById("scanner-output").innerText = "🎯 LETTO COMPLETATO: " + decodedText;
                ...
                Html5Qrcode.getCameras().then(devices => {
                    if (devices && devices.length > 0) {
                        // Forziamo parametri video avanzati per evitare sfocature
                        ...
                        .catch(err => {
                            document.getElementById("scanner-output").innerText = "⚠️ Errore di accesso. Controlla i permessi della fotocamera.";
                            document.getElementById("scanner-output").style.color = "#dc2626";
                        });
                    }
                }).catch(err => {
                    document.getElementById("scanner-output").innerText = "Nessuna fotocamera trovata su questo dispositivo.";
                });
            </script>
            """
