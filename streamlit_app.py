...
if st.session_state.ruolo_utente is None:
    ...
    # ==========================================
    # WORKFLOW 4: ALTRI MAGAZZINI (ATA / OFFICINA / ECC.)
    # ==========================================
    elif st.session_state.ruolo_utente == "magazziniere":
        st.markdown(f"## 📦 Magazzino Operativo: {st.session_state.magazzino_selezionato}")
        
        # Recupero i nomi dinamici delle schede dal dizionario MAPPA_SCHEDE basato sul magazzino loggato
        ...
        # Divisione dell'interfaccia in due tab pratici
        tab_inv, tab_req = st.tabs(["📋 INVENTARIO MERCI", "📥 RICHIESTE ASSEGNATE"])
        
        # --- SOTTO-SEZIONE INVENTARIO ---
        with tab_inv:
            ...
            if df_inventario_standard.empty:
                ...
                # Visualizzazione dell'inventario corrente
                st.dataframe(df_inventario_standard, use_container_width=True, hide_index=True)
                
                # Pannello per aggiornare le giacenze in tempo reale
                ...
                with st.form("form_giacenze_standard"):
                    ...
                    nuova_qta = st.number_input("Nuova quantità inserita a sistema:", min_value=0, step=1)
                    
                    if st.form_submit_button("Salva Nuova Giacenza"):
                        if "elemento" in df_inventario_standard.columns:
                            # Aggiorna il record locale e invia le modifiche allo sheet di riferimento
                            ...
                            st.success(f"Giacenza aggiornata con successo per: {elemento_sel}")
                            st.rerun()
                            
        # --- SOTTO-SEZIONE RICHIESTE ---
        with tab_req:
            st.subheader("Richieste Operative pervenute al Reparto")
            ...
            if df_richieste_standard.empty:
                ...
                # Visualizzazione delle richieste operative per il reparto loggato
                st.dataframe(df_richieste_standard, use_container_width=True, hide_index=True)
                
                st.markdown("### ⚙️ Cambia Stato Pratica / Ordine Interno")
                ...
                if id_list:
                    ...
                    with col_id:
                        id_sel = st.selectbox("Seleziona ID Richiesta da elaborare:", id_list, key="sel_std_id")
                    with col_st:
                        stato_sel = st.selectbox("Imposta Stato Avanzamento:", ["In Preparazione", "Pronto al Ritiro", "Consegnato", "Annullato"], key="sel_std_st")
                        
                    if st.button("Aggiorna Pratica", type="primary"):
                        # Trova l'indice della richiesta selezionata e aggiorna lo stato
                        ...
                        st.success(f"Richiesta {id_sel} aggiornata con successo in stato: {stato_sel}")
                        st.rerun()
