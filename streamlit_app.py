                                ...
                                with st.expander("Esegui Riconsegna ↩"):
                                    ...
                                    if metodo_firma_ric == "✍️ Disegna Firma Digitale":
                                        ...
                                        <div style="background: #f8fafc; border: 2px dashed #cbd5e1; padding: 15px; border-radius: 12px; max-width:510px; font-family: sans-serif;">
                                            ...
                                            <br>
                                            <div style="margin-top:10px; display:flex; gap:10px;">
                                                <button type="button" onclick="pulisciCanvasRic_{riga['id_comodato']}()" style="padding:8px 15px; background:#ef4444; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">Cancella</button>
                                                <button type="button" onclick="generaCodiceFirmaRic_{riga['id_comodato']}()" style="padding:8px 15px; background:#22c55e; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">Genera Codice Firma</button>
                                            </div>
                                        ...
                                        <script>
                                            var canvas_{riga['id_comodato']} = document.getElementById("canvas_ric_{riga['id_comodato']}");
                                            var ctx_{riga['id_comodato']} = canvas_{riga['id_comodato']}.getContext('2d');
                                            ctx_{riga['id_comodato']}.strokeStyle = '#000000';
                                            ctx_{riga['id_comodato']}.lineWidth = 3;
                                            ctx_{riga['id_comodato']}.lineCap = 'round';
                                            var isDrawing_{riga['id_comodato']} = false;

                                            function getCoordinate_{riga['id_comodato']}(e) {{
                                                var rect = canvas_{riga['id_comodato']}.getBoundingClientRect();
                                                if(e.touches && e.touches.length > 0) {{
                                                    return {{ x: e.touches[0].clientX - rect.left, y: e.touches[0].clientY - rect.top }};
                                                }}
                                                return {{ x: e.clientX - rect.left, y: e.clientY - rect.top }};
                                            ...
                                            canvas_{riga['id_comodato']}.addEventListener('mousedown', function(e) {{ isDrawing_{riga['id_comodato']} = true; var p = getCoordinate_{riga['id_comodato']}(e); ctx_{riga['id_comodato']}.beginPath(); ctx_{riga['id_comodato']}.moveTo(p.x, p.y); }});
                                            canvas_{riga['id_comodato']}.addEventListener('mousemove', function(e) {{ if(!isDrawing_{riga['id_comodato']}) return; var p = getCoordinate_{riga['id_comodato']}(e); ctx_{riga['id_comodato']}.lineTo(p.x, p.y); ctx_{riga['id_comodato']}.stroke(); }});
                                            canvas_{riga['id_comodato']}.addEventListener('mouseup', function() {{ isDrawing_{riga['id_comodato']} = false; }});

                                            function pulisciCanvasRic_{riga['id_comodato']}() {{ 
                                                ctx_{riga['id_comodato']}.clearRect(0, 0, canvas_{riga['id_comodato']}.width, canvas_{riga['id_comodato']}.height); 
                                                document.getElementById("output_b64_ric_{riga['id_comodato']}").style.display = 'none';
                                            }}

                                            function generaCodiceFirmaRic_{riga['id_comodato']}() {{
                                                var dataUrl = canvas_{riga['id_comodato']}.toDataURL('image/png');
                                        ...
                                        """
                                        st.components.v1.html(html_pad_ric, height=240)
                                        stringa_ric = st.text_area("Incolla qui il Codice Riconsegna:", value="", key=f"str_ric_{riga['id_comodato']}")
                                        if stringa_ric.startswith("data:image/png;base64,"):
                                            firma_ric_finale = stringa_ric
                                    else:
                                        file_ric = st.file_uploader("Carica immagine della firma:", type=["png", "jpg", "jpeg"], key=f"file_ric_{riga['id_comodato']}")
                                        if file_ric is not None:
                                            firma_ric_finale = "data:image/png;base64," + base64.b64encode(file_ric.read()).decode("utf-8")

                                    if st.button("↩ Conferma Riconsegna e Archivia", key=f"btn_ric_{riga['id_comodato']}", use_container_width=True):
                                        if not firma_ric_finale:
                                            st.error("Firma obbligatoria per completare lo scarico logistico.")
                                        else:
                                            with st.spinner("Processamento in corso..."):
                                                data_ora_ric = datetime.now().strftime("%d/%m/%Y %H:%M")
                                                pdf_bytes = genera_pdf_comodato(riga['id_comodato'], riga['nominativo'], riga['tipo_soggetto'], riga['id_bene'], data_ora_ric, "RICONSEGNA", firma_ric_finale, st.session_state.utente_corrente)
                                                nome_file_ric = f"Ricevuta_Riconsegna_{riga['id_comodato']}_{riga['nominativo'].replace(' ', '_')}.pdf"
                                                
                                                if carica_su_drive_unico(pdf_bytes, nome_file_ric, "application/pdf", "Comodati_Riconsegne"):
                                                    df_reg_comodati.loc[df_reg_comodati["id_comodato"] == riga['id_comodato'], "stato_comodato"] = f"Riconsegnato il {data_ora_ric}"
                                                    carica_su_sheet(df_reg_comodati, "Registro_Comodati")
                                                    
                                                    df_inv_comodati.loc[df_inv_comodati["id_bene"] == riga['id_bene'], "stato"] = "Disponibile"
                                                    carica_su_sheet(df_inv_comodati, "Inventario_Comodati")
                                                    
                                                    st.success("Riconsegna salvata su Registro e Google Drive!")
                                                    st.rerun()
                                                else:
                                                    st.error("Errore di archiviazione PDF.")
