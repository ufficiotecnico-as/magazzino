...
# ID Cartelle Destinazione Google Drive per l'Archiviazione
ID_CARTELLA_CONSEGNA = "1bVTs2smvVJONs2oIAFZdDvX9pYDK9MZT"  # Sostituire se differente
ID_CARTELLA_RICONSEGNA = "1bVTs2smvVJONs2oIAFZdDvX9pYDK9MZT" # Inserire ID cartella specifica riconsegna
...
def invia_email_sistema(destinatario, oggetto_mail, html_corpo):
    # Supporto flessibile sia per "email_config" che per la chiave diretta "smtp" come da log di errore
    cfg = None
...
def genera_pdf_riconsegna(id_contratto, nome, ruolo, bene, data, firma_base64=None, utente_loggato="Ufficio Tecnico"):
    ...
    pdf.set_font("Times", "B", 10)
    pdf.cell(95, 5, "", ln=False)
    pdf.cell(85, 5, f"Ai Docenti / Al Personale Interessato\nSig./Sigg. {nome} ({ruolo})", ln=True)
    pdf.ln(5)
    pdf.cell(22, 5, "OGGETTO: ", ln=False)
    pdf.set_font("Times", "", 10)
    pdf.multi_cell(158, 5, f"Ricevuta di Riconsegna, Scarico Logistico e Cessazione Comodato d'Uso - Registro ID {id_contratto}")
    pdf.ln(5)
    
    corpo = f"Con la presente si attesta che il bene sotto descritto è stato formalmente riconsegnato all'Istituto in data odierna, ponendo fine agli obblighi di custodia previsti dal contratto.\n\nDettaglio del Bene Riconsegnato:\n- Identificativo / Seriale: {bene}\n\nL'Ufficio Tecnico/Magazzino ha verificato l'integrità e lo stato del dispositivo, completandone l'operazione di scarico logistico dal registro dei comodati attivi."
    pdf.multi_cell(180, 6, pulisci_caratteri_fpdf(corpo))
    
    pdf.set_y(-50)
    y_f = pdf.get_y()
    pdf.cell(100, 5, f"Per l'Amministrazione:\nF.to {utente_loggato}")
    pdf.cell(80, 5, "Firma del Riconsegnante:")
...
if st.session_state.ruolo_utente is None:
    ...
    # ==========================================
    # WORKFLOW 3: ADMIN (CONSEGNA & RICONSEGNA)
    # ==========================================
            ...
            if pronte.empty: st.info("Nessun materiale o dispositivo in attesa di consegna fisica.")
                ...
                for _, riga in pronte.iterrows():
                    with st.expander(f"📦 ID {riga['id_richiesta']} - Consegna a {riga['richiedente']} [{riga['categoria_bene']}]"):
                        ...
                        # Utilizzo di un Form separato per evitare la perdita di stato della firma grafica incollata
                        with st.form(key=f"form_consegna_{riga['id_richiesta']}"):
                            ...
                            if sub_btn:
                                # Normalizzazione e verifica stringa base64 della firma
                                ...
                                if clean_firma and bene_assegnato:
                                    ...
                                    # Caricamento nella cartella corretta di DRIVE (CONSEGNA)
                                    if carica_su_drive_unico(pdf_bytes, f"Verbale_{id_com}_{riga['richiedente']}.pdf", "application/pdf", ID_CARTELLA_CONSEGNA):
                                        ...
                                        st.success("🎉 Pratica Evasa! Verbale di consegna registrato e archiviato in 'Consegna'.")
                                        st.rerun()
                                else:
                                    st.error("Errore: Verificare di aver inserito sia il Seriale/ID Bene sia il codice Firma valido.")

        # NUOVO SOTTO-MODULO: RICONSEGNA
        with tab_riconsegna:
            ...
            if comodati_attivi.empty:
                st.info("Nessun comodato risulta attualmente attivo nel sistema logistico.")
            else:
                for _, com in comodati_attivi.iterrows():
                    with st.expander(f"🔄 Reg ID: {com['id_comodato']} - {com['nominativo']} (Bene: {com['id_bene']})"):
                        with st.form(key=f"form_riconsegna_{com['id_comodato']}"):
                            ...
                            if sub_btn_ric:
                                ...
                                if clean_firma_ric:
                                    ...
                                    # Generazione della ricevuta ufficiale (vedi layout immagine 4)
                                    ...
                                    # Caricamento nella cartella corretta di DRIVE (RICONSEGNA)
                                    if carica_su_drive_unico(pdf_ric_bytes, f"Ricevuta_Riconsegna_{com['id_comodato']}_{com['nominativo']}.pdf", "application/pdf", ID_CARTELLA_RICONSEGNA):
                                        ...
                                        # Aggiorna Stato sul Registro Storico dei Comodati
                                        ...
                                        # Ripristina lo stato del dispositivo a "Disponibile" nell'inventario
                                        df_inv_c = scarica_da_sheet("Inventario_Comodati")
                                        ...
                                        st.success(f"🔄 Bene {com['id_bene']} ritornato in magazzino. Ricevuta salvata in 'Riconsegna'.")
                                        st.rerun()
                                else:
                                    st.error("Inserire la firma per completare lo scarico.")
