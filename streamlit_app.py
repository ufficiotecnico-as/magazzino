...
# ==========================================
# Associa ogni password specifica al rispettivo magazzino
PASSWORD_MAP = {
...
def carica_su_drive(file_bytes, nome_file, mime_type, nome_magazzino):
    ...
    try:
        ...
        # Cerca o crea la sottocartella specifica del magazzino corrente
        ...
        # Caricamento del file nella cartella corretta
        file_metadata = {
            'name': nome_file, 
...
# ==========================================
# INIZIALIZZAZIONE DATABASE CON MULTI-MAGAZZINO
# ==========================================
        ...
        with st.container():
            ...
            if scelta_accesso == "Sono un Collaboratore (Fai una Richiesta)":
                ...
                if st.button("Verifica Credenziali"):
                    # Controllo se la password appartiene a un magazziniere specifico
                    if password_input in PASSWORD_MAP:
                        ...
                        st.rerun()
                    # Controllo se la password è dell'admin globale
...
# --- INTERFACCE ABILITATE ---
    ...
    # --- 1. SCHERMATA COLLABORATORE (Invia richieste ai magazzini) ---
    ...
    # --- 2. SCHERMATA MAGAZZINIERE (PROFILATA SULLA SUA PASSWORD) ---
        ...
        tab_consegne, tab_carico, tab_ddt = st.tabs(["📋 Richieste in Arrivo", "➕ Carico / Inventario", "📸 Archivia Foto DDT"])
        
        # TAB 1: Evasione richieste relative SOLO al magazzino sbloccato
        with tab_consegne:
                        ...
                        with c3:
                            if st.button("Consegna ✔", key=f"btn_{row['id_richiesta']}"):
                                ...
                                if filtro.any():
                                    ...
                                    st.error("⚠️ Questo è un articolo personalizzato ('Altro'). Censiscilo nell'inventario prima di consegnarlo.")

        # TAB 2: Inventario isolato del magazzino sbloccato
        with tab_carico:
            ...
            with st.container():
                ...
                nuovo_id_art = st.text_input("Codice Articolo (es. ATA05, OFF12):")
        ...
        # TAB 3: Archiviazione DDT protetta nella cartella specifica
        with tab_ddt:
            st.markdown(f"### 📸 Archiviazione DDT - Sottocartella: DDT_{mag_corrente.replace(' ', '_')}")
            with st.container():
                foto_ddt = st.camera_input("Inquadra e scatta")
                ...
                if file_da_elaborare is not None:
                    ...
                    if st.button("Invia ed Archivia su Google Drive 🚀"):
                        with st.spinner("Salvataggio in corso..."):
                            ...
                            if id_drive: 
                                st.success(f"✔️ Archiviato nel Drive condiviso sotto la cartella di {mag_corrente}!")

    # --- 3. SCHERMATA AMMINISTRATORE (VEDE TUTTO E FILTRA) ---
    elif st.session_state.ruolo_utente == "admin":
